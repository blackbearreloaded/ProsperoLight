...
static void load_asset_message(void)
{
    ...
    long count = read(descriptor, asset_message, sizeof(asset_message) - 1);
    close(descriptor);
    if (count <= 0)
    {
        copy_message(asset_message, sizeof(asset_message), "APP0 ASSET FAILED");
        return;
    }
...
static void put_pixel(uint32_t *pixels, unsigned x, unsigned y, uint32_t color)
{
    size_t offset = tiled_byte_offset(x, y);
    *(uint32_t *)((uint8_t *)pixels + offset) = color;
}

static void fill_rect(uint32_t *pixels, unsigned x, unsigned y, unsigned width, unsigned height,
                      uint32_t color)
{
...
static void fill_circle(uint32_t *pixels, unsigned center_x, unsigned center_y, unsigned radius,
                        uint32_t color)
{
    ...
    for (int y = -signed_radius; y <= signed_radius; ++y)
    {
        for (int x = -signed_radius; x <= signed_radius; ++x)
        {
            ...
            {
                put_pixel(pixels, center_x + (unsigned)x, center_y + (unsigned)y, color);
            }
        }
    }
}

static void fill_triangle(uint32_t *pixels, unsigned center_x, unsigned top, unsigned half_width,
                          unsigned height, uint32_t color)
{
    for (unsigned row = 0; row < height; ++row)
    {
        unsigned half = row * half_width / height;
        fill_rect(pixels, center_x - half, top + row, half * 2 + 1, 1, color);
    }
}

static void draw_text(uint32_t *pixels, unsigned x, unsigned y, const char *text, unsigned scale,
                      uint32_t color)
{
...
int main(void)
{
    ...
    int result =
        sceKernelAllocateDirectMemory(0, (int64_t)pool_size, MEMORY_BYTES, MEMORY_ALIGNMENT,
    ...
    result = sceVideoOutRegisterBuffers2(video, 0, 0, buffers, 2, &attribute, 0, NULL);
    if (result < 0)
